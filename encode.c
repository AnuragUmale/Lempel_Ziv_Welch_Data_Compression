...
void usage(void) {
    ...
    fprintf(stderr, "   Compressed files are decompressed with the corresponding decoder.\n\n");
    ...
    fprintf(stderr, "   -o output   Specify output of compressed input (stdout by default)\n");
    fprintf(stderr, "   -h          Display program usage\n");
}

void verb(void) {
    double saving = 1.0 - ((float) byte(total_bits) / (float) total_syms); 
